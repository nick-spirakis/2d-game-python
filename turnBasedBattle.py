...
class TurnBasedBattle:
    ...
    def draw_menu(self, option_colors): #main menu
...
#------------------------------------------------------------------------------------------------------------------
                        if selected_action == "Attack":
                            ...
                            self.open_attack()
                            self.selected_attack = 0 #this number determines the starting blue
...
#---------------------------------------------------------------------------------------------------------------------
                            ...
                            self.open_magic()
                            self.selected_magic = 0 #this number determines the starting blue
...
# --------------------------------------------------------------------------------------------------------------------
                            ...
                            self.open_item()
                            self.selected_item = 0 #this number determines the starting blue
...
# magic --------------------------------------------------------------------------------------------------------------------------------------------------
                    ...
                    if event.key == pygame.K_w:
                        ...
                        if selected_magic_option == "Magic 1":
                            print("magic 1")
                            self.close_magic()
                            self.is_player_turn = False
                            self.magic_menu_nav = False
                        ...
                        if selected_magic_option == "Magic 2":
                            print("magic 2")
                            self.close_magic()
                            self.is_player_turn = False
                            self.magic_menu_nav = False 
                        ...
                        if selected_magic_option == "Magic 3":
                            print("magic 3")
                            self.close_magic()
                            self.is_player_turn = False
                            self.magic_menu_nav = False
...
# ********************************************************************************
    ...
    def return_to_level(self):
        if self.level.levelNum == 5:
            ...
            # Handle other level transitions here if i want
            pass

    
    def display_attack_name(self, attack_name, enemy_image): #added enemy_image
        menu_background = pygame.Surface((self.menu_width, self.menu_height))
        menu_background.fill((0, 0, 255))
        pygame.draw.rect(menu_background, (255,255,255), menu_background.get_rect(), 2)
        ...
        # Render and display the attack name on the battle surface
        attack_font = pygame.font.Font(None, 36)
        attack_text = attack_font.render(attack_name, True, (255, 255, 255))
        
        #attack_rect = attack_text.get_rect(center=(self.menu_x + self.menu_width // 2, self.menu_y - 30))

        text_x = self.menu_x + self.menu_width - attack_text.get_width() - 30 #-10
        ...
        enemy_rect = enemy_image.get_rect()  # Adjust the position as needed
        enemy_rect.topleft = (self.menu_x + 10, self.menu_y - 47)  # Adjust the position as needed
    ...
    def run(self):
        ...
        while True:  # Main battle loop start
            ...
            if self.escape == True:
                ...
                break #this returns us to level 3
